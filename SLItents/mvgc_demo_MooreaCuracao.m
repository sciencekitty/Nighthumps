...
for i = 1:length(files)
    ...
    X = [manta.DOXY,manta.moon,manta.TF,manta.Hum,manta.Pres,manta.Wind,manta.WindDir];

    times=datetime(manta.SDN,'ConvertFrom','datenum');
    vars={'DOXY','moon','TF','Hum','Pres','Wind','WindDir'};
...
%     plot_spw(f,fs);
    ...
    save([island_name,'_GC.mat'],'G','F','f','sig','pval','info');
    writetable((array2table(pval,'VariableNames',vars)),[island_name,'-pval.csv']);
    writetable((array2table(sig,'VariableNames',vars)),[island_name,'-sig.csv']);
